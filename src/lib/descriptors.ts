/**
 * IELTS Writing Task 2 band descriptors — the official text.
 *
 * Source: "Writing Band Descriptors", updated May 2023, published by IELTS at
 *   https://ielts.org/cdn/Guides/ielts-writing-band-descriptors.pdf
 * Extracted from that PDF, Task 2 table, bands 9 down to 4. Bands 3 and below
 * are omitted: they describe responses far below anything you will produce, and
 * every token here is sent on every marking call.
 *
 * IELTS's own note on how to read this table is reproduced for the marker:
 * a script must FULLY fit the positive features of a descriptor to be awarded
 * that band.
 */

export const TASK2_DESCRIPTORS = `A script must fully fit the positive features of the descriptor at a particular level to be awarded that band.
...
Band 4: A very limited range of structures is used. Subordinate clauses are rare and simple sentences predominate. Some structures are produced accurately but grammatical errors are frequent and may impede meaning.
`.trim();
